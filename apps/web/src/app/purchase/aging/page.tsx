...
import { getApAgingSummary } from '@/app/actions/commercial-docs';
...
export default async function ApAgingPage({
  ...
  let aging;
  try {
    [tenant, aging] = await Promise.all([getTenantInfo(), getApAgingSummary(asOf)]);
  } catch {
    redirect('/login');
  }
  ...
  return (
    <BookOneShell active="AP Aging" tenant={tenant}>
      <div className="workspace party-workspace">
        ...
        >
          {(Object.keys(BUCKET_LABEL) as Array<keyof typeof BUCKET_LABEL>).map((key) => {
            ...
            const href = active
              ? `/purchase/aging?asOf=${asOf}`
              : `/purchase/aging?asOf=${asOf}&bucket=${key}`;
            return (
              <Link key={key} href={href} style={{ textDecoration: 'none', color: 'inherit' }}>
                <Card>
                  <div className="card-body" style={{ padding: 12 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: active ? 'var(--brand)' : 'var(--ink-soft)' }}>
                      {BUCKET_LABEL[key]}
                    </div>
          ...
          <Card>
            <div className="card-body" style={{ padding: 12 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--ink-soft)' }}>Total open AP</div>
              <div style={{ fontSize: 16, fontWeight: 800, marginTop: 4 }}>{formatLKR(aging.grandTotal)}</div>
            </div>
          </Card>
        </div>
          ...
          <div className="card-body" style={{ padding: 0 }}>
            {rows.length === 0 ? (
              <div className="empty-state" style={{ padding: 28 }}>
                <h3>No open payables</h3>
                <p>All bills are paid, or none posted yet.</p>
              </div>
            ) : (
                ...
                <table className="table">
                  ...
                  <tbody>
                    {rows.map((r) => (
                      <tr key={r.id}>
                        <td>
                          <Link href={`/purchase/purchases/${r.id}`}>
                            <strong>{r.documentNumber}</strong>
                          </Link>
                          <div style={{ fontSize: 11, color: 'var(--ink-soft)' }}>{r.documentType}</div>
                        </td>
                        <td>{r.partyName}</td>
                        <td>{r.agingDate}</td>
                        <td>{r.daysPastDue}</td>
                        ...
                        <td>
                          <Link href={`/purchase/payments/new?documentId=${r.id}`}>
